...
class Magazine extends PhModel
{
    ...
    protected function deleteListCache(){
        ...
        $listCache = $cache->queryKeys(self::CACHE_ZINE_LIST_KEY);
        foreach($listCache as $val){
            $cache->delete( $val );
        }

        $singleCache = $cache->queryKeys(self::CACHE_ZINE_KEY . $this->id);
    ...
    static public function get($nid){
        ...
        return self::findFirst(array(
            ...
            'cache' => array('key' => self::CACHE_ZINE_KEY . $nid)
        ));
    }
    ...
    static public function getPrevious($nid){
        ...
        return self::findFirst(array(
            ...
            'bind' => array('nid' => $nid, 'status' => 1 ),
            'order' => 'id DESC',
            'cache' => array('key' => self::CACHE_ZINE_KEY . $nid . ':previous')
        ));
    }
    ...
    static public function getNext($nid){
        ...
        return self::findFirst(array(
            'id>:nid: and status=:status:',
            'bind' => array('nid' => $nid, 'status' => 1 ),
            'cache' => array('key' => self::CACHE_ZINE_KEY . $nid . ':next')
        ));
    }
    ...
    public function getList($number, $offset, $category){
        $param = array(
            ...
            'cache' => array('key' => self::CACHE_ZINE_LIST_KEY . "{$category}:{$number}:{$offset}")
        );
        ...
        foreach($records as &$row){
            $cachekey = self::CACHE_ZINE_TIMES_KEY . $row['id'];
    ...
    public function increment($id, $times=1){
        ...
        $key = self::CACHE_ZINE_TIMES_KEY . $id;
